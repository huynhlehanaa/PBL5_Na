...
def run_layout(input_dir: Path, output_dir: Path):
    model_path = "doclayout_yolo_docstructbench_imgsz1024.pt"
    full_model_path = str(Path(__file__).resolve().parent.parent / model_path)
    
    # Khởi tạo model bằng class YOLOv10
    model = YOLOv10(full_model_path) 
    ...
    for p in files:
        # Luồng dự đoán chuẩn của YOLOv10 sẽ tự động xử lý dict mà không gọi NMS
