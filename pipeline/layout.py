...
def _select_device():
    ...
    try:
        ...
        raise ImportError(
            "Thiếu torch. Cài đặt nhanh: pip install torch"
        )
...
def run_layout(
    ...
    - input_dir: thư mục chứa ảnh đã tiền xử lý.
    - output_dir: lưu ảnh annotate + layouts.json.
    - model_path: đường dẫn model (.pt) hoặc repo HF. Mặc định dùng
    ...
    try:
        ...
        raise ImportError(
            "Thiếu doclayout-yolo. Cài đặt: pip install doclayout-yolo"
        ) from exc
